...
export const groupRegistrationService = {
  ...
  registerTeam: data => {
    ...
    const alreadyRegistered = registrations.some(r => r.eventId === data.eventId && r.teamLeader.email === data.teamLeaderEmail);
    if (alreadyRegistered) {
      throw new Error('Your team is already registered for this event');
    }

    // Check if team name is unique for this event
    const teamNameExists = registrations.some(r => r.eventId === data.eventId && r.teamName.toLowerCase() === data.teamName.toLowerCase());
    if (teamNameExists) {
  ...
  isTeamRegistered: (eventId, email) => {
    const registrations = groupRegistrationService.getAllGroupRegistrations();
    return registrations.some(r => r.eventId === eventId && r.teamLeader.email === email);
  },
  // Cancel team registration
  cancelTeamRegistration: registrationId => {
    const registrations = groupRegistrationService.getAllGroupRegistrations();
  ...
  updateTeamRegistration: (registrationId, updates) => {
    const registrations = groupRegistrationService.getAllGroupRegistrations();
    const index = registrations.findIndex(reg => reg.id === registrationId);
