...
class EmailServiceClass {
  ...
  async sendTicketConfirmationEmail(userEmail, userName, eventTitle, eventDate, eventLocation, ticketNumber, verificationCode, qrCodeDataUrl) {
    try {
      const htmlBody = this.generateTicketEmailHTML(userName, eventTitle, eventDate, eventLocation, ticketNumber, verificationCode, qrCodeDataUrl);
      const plainText = this.generatePlainTextEmail(userName, eventTitle, eventDate, ticketNumber, verificationCode);
      const emailLog = {
        to: userEmail,
  ...
  generateTicketEmailHTML(userName, eventTitle, eventDate, eventLocation, ticketNumber, verificationCode, qrCodeDataUrl) {
    return `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <div class="container">
          ...
          <div style="color: #333; line-height: 1.6;">
            ...
            <p style="text-align: center; margin-top: 30px;">
              <a href="${typeof window !== 'undefined' ? window.location.origin : 'https://campusconnecthub.com'}/my-tickets" style="background: #4f46e5; color: white; padding: 12px 30px; border-radius: 5px; text-decoration: none; display: inline-block;">View My Tickets</a>
            </p>
          </div>
          <div class="footer">
  ...
  generatePlainTextEmail(userName, eventTitle, eventDate, ticketNumber, verificationCode) {
    return `
...
support@campusconnecthub.com
  ...
  async unsubscribe(email) {
    try {
      const subscriptions = localStorage.getItem(SUBSCRIPTIONS_KEY);
      const subscribedEmails = subscriptions ? JSON.parse(subscriptions) : [];
      const filtered = subscribedEmails.filter(e => e !== email);
  ...
  async sendRegistrationConfirmation(userEmail, userName, eventTitle, eventDate) {
    try {
      const htmlBody = `
        <!DOCTYPE html>
        <html>
