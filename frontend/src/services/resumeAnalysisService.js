...
export const resumeAnalysisService = {
  ...
  extractSkillsFromResume: resumeText => {
    const commonSkills = ['java', 'python', 'javascript', 'typescript', 'react', 'node', 'nodejs', 'html', 'css', 'sql', 'mongodb', 'postgresql', 'mysql', 'rest', 'api', 'aws', 'azure', 'docker', 'kubernetes', 'git', 'linux', 'windows', 'c++', 'c#', 'php', 'ruby', 'golang', 'rust', 'scala', 'kotlin', 'machine learning', 'ml', 'ai', 'deep learning', 'tensorflow', 'pytorch', 'django', 'flask', 'spring', 'hibernate', 'jpa', 'servlet', 'jsp', 'angular', 'vue', 'nextjs', 'express', 'fastapi', 'graphql', 'agile', 'scrum', 'jira', 'jenkins', 'ci/cd', 'devops', 'communication', 'leadership', 'teamwork', 'problem solving', 'data structures', 'algorithms', 'design patterns', 'oops', 'oop', 'microservices', 'monolith', 'rest api', 'soap', 'websocket'];
    const foundSkills = [];
    const seen = new Set();
  ...
  extractCGPAFromResume: resumeText => {
    // Look for patterns like "CGPA: 8.5" or "GPA: 3.8" or "8.5/10"
    const cgpaPatterns = [/cgpa\s*[:=]?\s*(\d+\.?\d*)/i, /gpa\s*[:=]?\s*(\d+\.?\d*)/i, /(\d+\.?\d*)\s*\/\s*10/i, /(?:score|average)\s*[:=]?\s*(\d+\.?\d*)/i];
  ...
  analyzeResume: (resumeText, requiredSkills, requiredCGPA) => {
    ...
    const matchedSkills = requiredSkills.filter(skill => resumeSkills.some(resSkill => resSkill.toLowerCase().includes(skill.toLowerCase()) || skill.toLowerCase().includes(resSkill.toLowerCase())));
    ...
    const skillsScore = requiredSkills.length > 0 ? matchedSkills.length / requiredSkills.length * 60 : 60;
  ...
  generateAISuggestions: (missingSkills, eligibilityScore, userCGPA, requiredCGPA) => {
    const suggestions = [];
  ...
  getAntAPIAnalysis: async (resumeText, jobDescription) => {
    try {
      ...
      const improvementAreas = ['Strengthen technical skill set', 'Add more project experience', 'Improve communication in resume'];
