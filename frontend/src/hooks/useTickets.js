...
export const useTickets = () => {
  ...
  const getActiveTickets = useCallback(userId => {
    try {
  ...
  // Get event tickets
  const getEventTickets = useCallback(eventId => {
    try {
  ...
  const createTicket = useCallback(async (eventId, eventTitle, eventDate, eventLocation, userId, userEmail, userName, registrationId, paymentId, qrCodeDataUrl, eventStartTime, seat, tier) => {
    try {
      setLoading(true);
      setError('');
      ...
      const emailResult = await emailService.sendTicketConfirmationEmail(userEmail, userName, eventTitle, eventDate, eventLocation, ticket.ticketNumber, ticket.verificationCode, qrCodeDataUrl);
  ...
  // Verify ticket
  const verifyTicket = useCallback((ticketId, verificationCode) => {
    try {
  ...
  const hasTicketForEvent = useCallback((userId, eventId) => {
    return ticketService.hasTicketForEvent(userId, eventId);
  }, []);

  // Get event stats
  const getEventStats = useCallback(eventId => {
