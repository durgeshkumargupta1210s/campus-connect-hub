...
import { clubApplicationService } from '@/services/clubApplicationService';
export const useClubApplications = () => {
  ...
  const getApplicationsByClubId = useCallback(clubId => {
    return clubApplicationService.getApplicationsByClubId(clubId);
  }, []);
  const addApplication = useCallback(applicationData => {
    try {
      setError(null);
      setLoading(true);
